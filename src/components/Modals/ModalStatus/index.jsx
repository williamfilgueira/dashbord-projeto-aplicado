  ...
  return (
    ...
    >
      ...
      <ContainerEmojiPicker>
        ...
        <ContainerInput>
          <Input required placeholder=":)"  size='small'  value={chosenEmoji.emoji} onChange={(event) => setChosenEmoji(event.target.value)}/>
          <Input required placeholder="Status" value={statusName} onChange={(event) => setStatusName(event.target.value)}/>
        </ContainerInput>
        <ButtonCommon maincolor='blue' title="CADASTRAR" onClick={() => handleStatus(chosenEmoji, statusName)} />
      </FormAddMember>
    </BaseModal>
  );
}
